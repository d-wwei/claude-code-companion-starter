...
| 文件 | 用途 |
| --- | --- |
| [`step1-init.md`](./step1-init.md) | Bootstrap 提示词——复制 `text` 代码块发给 Claude Code 即可 |
| [`claude-code-personal-assistant-bootstrap-prompt.md`](./claude-code-personal-assistant-bootstrap-prompt.md) | 提示词说明：做了什么、写了哪些文件、包含哪些优化 |
...
> **只需一次。** 全局规则写入后永久生效。以后进入任何新项目，Claude Code 会自动初始化 `.assistant/` 并进入 bootstrap。
...
有了 `.assistant/`，这些变成可编辑的项目资产：

| 文件 | 存储内容 |
| --- | --- |
| `USER.md` | 用户画像：姓名、角色、上下文 |
| `STYLE.md` | 沟通风格：简洁 vs 详细、语气 |
| `WORKFLOW.md` | 工作方式：报告结构、决策偏好 |
| `MEMORY.md` | 长期可复用的事实和偏好 |
| `memory/projects/*.md` | 项目级决策和跨会话上下文 |
| `memory/daily/*.md` | 临时日常笔记（自动生命周期：7 天→提升，14 天→建议删除） |
| `runtime/inbox.md` | 短期待办事项和提醒 |
| `runtime/last-session.md` | 上次会话摘要和下一步 |
