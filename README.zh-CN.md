...
- **Agent 自我认知**：Agent 知道自己是谁——名字、性格、价值观、运行环境和沟通渠道——持久化在 `global-agent-soul.md` 中，并可通过项目级 `AGENT.md` 进行覆盖。
...
| `global-agent-soul.md` | Agent 自我认知：身份、价值观、性格、边界、运行环境 |
...
.assistant/
  ...
  AGENT.md           — [可选覆盖] 项目级 Agent 角色/性格；缺省时继承 global-agent-soul.md
...
20. Agent 自我认知层（`global-agent-soul.md` + 项目级 `AGENT.md`）
