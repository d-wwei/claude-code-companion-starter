...
.assistant/
  ...
  runtime/
    ...
    active-task.md   — 当前主任务恢复锚点
    interrupted-tasks.md — 按优先级排列的中断任务队列
    resume-protocol.md — 首条恢复回复的硬规则
    resume-checkpoint-template.md — 命名断点模板
```
...
当用户说 `continue`、`resume`、`刚才做到哪里了`、`恢复刚才的任务` 这类话时，Claude 应先读 `active-task.md`，然后输出一个三段式恢复回复：
